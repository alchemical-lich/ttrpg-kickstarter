...
Is this drop driven by cheap zines entering the Kickstarter market and dragging the median down? Partly, but not completely. ZineQuest (from 2019) did pull in a flood of cheap zines, and dropping them lifts the median a little, but not by that much: among **non-zine** books the real per-backer pledge still dropped about **45%**, and within the subcategory of books that **name D&D 5e** — about **56%** (with the real raise down **62%**). Excluding the cheapest type of project doesn't change the overall picture, so it’s not just compositional effects.
...
I think this suggests an interesting shift the Kickstarter market over time. While any *given* tier held its real value, the *typical project* still raised less — because campaigns now spread their backers across more, and cheaper, tiers (e.g., a $1–15 PDF or an entry pledge below the hardback), and because more, smaller projects launch every year. The same project with a hardback option now reaches a much broader base of cheap-tier backers, which drags the *average* outlay down even as its own price holds. At the **population** level the average RPG product really is cheaper now — but only because cheap formats like zines and PDF-only releases proliferated, which is composition again, not discounting. RPG products didn't get cheaper *per se* but the available *mix* of projects did. Creators have also set their asks to match — the goals they choose have fallen even faster in real terms, which I come back to below.
