...
Two further points. First, I fed each campaign's *text* — titles and blurbs — into the model, restricting to *books* (in the pooled book-plus-accessory sample, physical-product words like "miniatures" and "scenery" mostly flag the accessory class rather than a wording effect). The text adds modestly to predictive power, but the strongest individual terms are largely a caution about the method: they are **brand and series names** — *Mothership*, *Forbidden Lands*, *Root*, *DCC*, *Dimgaard* — which a bag-of-words model can memorize as "these named lines did well" without the lesson generalizing to a new project.[^text] Setting brand names aside, the generalizable pattern is a **premium-versus-commodity axis**: "softcover," "official," "award," and "supplement" predict raising *more*, while "pay what you want," "one-shot," "online," and "ttrpg inspired" predict raising *less*. Presentation as a complete, premium object is associated with larger raises; cheap or pay-what-you-want framing is not.

![Words that predict raising more vs less](images/text_top_terms.png)

*Title/blurb terms predicting how much a funded RPG **book** raises (LASSO, controlling for the structured features). Note the caveat: several of the strongest "raises more" terms (blue) are specific brand and series names the model has memorized; the generalizable signal is premium, complete-product language versus pay-what-you-want and one-shot framing (red).*
...
[^text]: Method, briefly: among funded RPG books, I turned titles and blurbs into a bag of words and bigrams and let a LASSO pick which ones predict log-dollars, with the vocabulary chosen separately inside each cross-validation fold so the test data couldn't leak in. Adding text lifted out-of-sample R² from about **0.68 to 0.70** — real but modest. The leading caveat: a bag-of-words model memorizes specific successful *series and brand names* (here Mothership, Forbidden Lands, Root, DCC, and Dimgaard load most strongly), which does not generalize to new campaigns; the part worth keeping is the premium/complete-product-versus-commodity pattern. I restrict this to books because in the pooled book-and-accessory version the strong terms were physical-product words (diorama, scenery, minis) that mostly identify the accessory class rather than a wording strategy.
